...
view: data_intelligence_ar {
  ...
  measure: Sales_Total_DSO {
    ...
    #sql:
    #CASE
    #when CAST(${Posting} as Date)<= CAST(CURRENT_DATE() as Date) and CAST(${Posting} as Date)>= DATE_SUB(${Current_Fiscal_Date_date},INTERVAL {% parameter Day_Sales_Outstanding %} MONTH )
    #THEN ${Sales_Global_Currency}
    #END;;
    }
